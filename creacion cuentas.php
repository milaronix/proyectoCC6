						...
						<form class="form-horizontal" method = 'post' action='<?php $_SERVER['PHP_SELF'] ?>'>
							<fieldset>
								...
								<div class="control-group <?php if($_POST['enviado'] == 1 && $_POST['bin'] == ''){ echo("error");} ?>">
									...
									<div class="controls">
										<select id="bin" name='bin' data-rel="chosen" <?php if(isset($_POST['bin'])){echo("value = '" . $_POST['bin'] . "'");} ?>>
											<?php
											$query = 'select * from bin';
											$resultado = mysql_query($query);
											while($items = mysql_fetch_array($resultado)){
												echo("<option value='$items[bin]'>");
												//if(isset($_POST['enviado'])){if($_POST['bin'] == $items['idbin']){echo(" selected ");};}
												echo("$items[binId] - $items[descripcion]</option>");
											}
											?>
										</select>
									</div>
